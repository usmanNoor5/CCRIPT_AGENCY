...
def build_crop_question(qw: int, qh: int) -> str:
    return (
        ...
        "Return the bounding box of drawing section only — room outlines and walls. "
        "CRITICAL: Do NOT include any tables, schedules, or data grids. "
        "Exclude completely: any table at the bottom (door schedule, window schedule, room finish, "
        "revision table, notes table, legend table). Exclude any table or column of boxes on the "
        "right side (schedules, legends, key plans). The crop must end at the edge of the "
        "drawing itself; do not include title block, scale bar, north arrow, or margins. "
        "If both FOUNDATION and FLOOR PLAN are visible, choose the FLOOR PLAN region only. "
        "The floor plan is typically the large open drawing area (left or centre); tables and "
        "schedules are separate regions — leave them out. "
        f"Output ONLY a JSON array of 4 integers [x1, y1, x2, y2] in pixel coords for this {qw}x{qh} image, "
        "where the box tightly encloses just the floor plan drawing and nothing else. "
...
def get_vlm_crop_bbox(
    model, processor, image_path: Path, img: np.ndarray, padding: int = 20
) -> tuple[int, int, int, int] | None:
    """
    Ask VLM for the bounding box of the architectural drawing only (no tables/schedules).
    Returns (x1, y1, x2, y2) in **original image pixel coords**, or None if parse failed.
    Callers can use this to crop the image or to map to PDF coordinates for tiling/vectors.
    """
    ...
    try:
        ...
        if image_path.name.startswith("_vlm_tmp_"):
            pass  # avoid noisy print when called from internal temp
        else:
    ...
    if bbox_q is None:
        return None
    x1 = max(0,      int(bbox_q[0] * sx) - padding)
    y1 = max(0,      int(bbox_q[1] * sy) - padding)
    x2 = min(orig_w, int(bbox_q[2] * sx) + padding)
    y2 = min(orig_h, int(bbox_q[3] * sy) + padding)
    return (x1, y1, x2, y2)
...
def vlm_crop(model, processor, image_path: Path, img: np.ndarray) -> np.ndarray:
    ...
    orig_h, orig_w = img.shape[:2]
    bbox = get_vlm_crop_bbox(model, processor, image_path, img, padding=20)
    if bbox is None:
        print("    Could not parse bbox — using full image.")
        return img
    x1, y1, x2, y2 = bbox
    print(f"    Original crop: [{x1},{y1},{x2},{y2}] (img {orig_w}x{orig_h})")
